...
- **Choice:** make near-neighbor descriptions contrastive —
...
- Keep raw answers (probabilities, confidence, resolved `model`) in storage —
  re-weighting and re-thresholding later must not re-run inference.
...
- **Band nouls, don't binarize.** `>0.7 act`, `0.3–0.7 review`, `<0.3 skip` —
  the uncertain band IS the product (self-consistency cookbooks).
