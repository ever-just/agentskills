...
9. **Hard-scope every personal data store before you touch it.** Mail archives, message histories and browser profiles are the richest source of vendor-account evidence — and the easiest place to massively overreach. "Search the mail for X" authorizes reading *messages matching X*, not indexing the archive. Specify: which accounts, which mailboxes, matched against which terms, and that the working set is **the matching messages only**. Forbid whole-store indexes, and forbid writing derived copies (a `.sqlite` envelope index or a headers CSV of 100k messages is a copy of the user's private correspondence sitting in a temp directory). Delete any bulk artifact when the task ends. On a real run an agent told to find vendor accounts built a 245 MB copy of the full mail index across nine personal accounts, including accounts unrelated to the business — because the prompt bounded the *output* (headers only) and never bounded the *corpus*. Bound both.
